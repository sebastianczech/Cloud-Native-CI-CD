...
def localstack_config_s3(localstackConfig):
...
if __name__ == "__main__":
    ...
    s3_create_bucket(localstack_config_s3(localstack_config()), 'demo-bucket-py')

    # Upload binary data
    s3_upload_file(localstack_config_s3(localstack_config()), 'demo-bucket-py')

    # Print out bucket names
    print("\nS3 buckets:")
    for bucket in s3_list_buckets(localstack_config_s3(localstack_config()))['Buckets']:
        print("- " + bucket['Name'])

    # Print out bucket files
    print("\nFiles in demo-bucket-py:")
    for obj in s3_list_object_in_bucket(localstack_config_s3(localstack_config()), 'demo-bucket-py'):
        print("- " + obj['Key'])
